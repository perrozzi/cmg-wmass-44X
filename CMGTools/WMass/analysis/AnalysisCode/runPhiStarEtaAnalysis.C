...
int main(int argc, char ** argv) 
{
  ...
  cout << "processing line "<< Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d)",ZfileDATA.Data(),ZfileDATA_lumi_SF,sample.Contains("DYJetsSig")?useAlsoGenPforSig:0) << endl;
  ...
  gROOT->ProcessLine(Form(".! mv ../%s/PhiStarEtaAnalysis.root ../%s/%s.root",filename_outputdir.Data(),filename_outputdir.Data(),outputfilename.Data()));
  gROOT->ProcessLine(Form(".! cp PhiStarEtaAnalysis.* ../%s/",filename_outputdir.Data()));

}


// void runPhiStarEtaAnalysis(TString ZfileDATA, double ZfileDATA_lumi_SF, TString sample, int useAlsoGenPforSig, int IS_MC_CLOSURE_TEST, int isMCorDATA, TString filename_outputdir, int useMomentumCorr, int smearRochCorrByNsigma, int useEffSF, int usePileupSF, int run_BuildEvByEvTemplates, int controlplots)
// {

  // // gSystem->CompileMacro("MuScleFitCorrector.cc");
  // // gSystem->CompileMacro("rochcor_44X_v3.C");
  // // gSystem->CompileMacro("rochcor_42X.C");
  // // gSystem->CompileMacro("PhiStarEtaAnalysis.C","kO-");
  // gROOT->ProcessLine(".L rochcor_44X_v3.C+");
  // gROOT->ProcessLine(".L PhiStarEtaAnalysis.C+");
  // cout << "processing line "<< Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d)",ZfileDATA.Data(),ZfileDATA_lumi_SF,sample.Contains("DYJetsSig")?useAlsoGenPforSig:0) << endl;
  // gROOT->ProcessLine(Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d)",ZfileDATA.Data(),ZfileDATA_lumi_SF,sample.Contains("DYJetsSig")?useAlsoGenPforSig:0));
  ...
  // // // if(IS_MC_CLOSURE_TEST and sample.contains("DYJetsSig")){
    ...
    // // // gROOT->ProcessLine(Form(".! mv ../%s/PhiStarEtaAnalysis.root ../%s/PhiStarEtaAnalysisOnDATA.root",filename_outputdir2.Data(),filename_outputdir2.Data()));
  // // // }




  // // gSystem->CompileMacro("PhiStarEtaAnalysis.C");
  // // cout << "processing line "<< Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d)",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0) << endl;
  // // // gROOT->ProcessLine(Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d); zDATA.Loop(%d,%d,\"../%s\",%d,%d,%d,%d,\"%s\")",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0,IS_MC_CLOSURE_TEST,isMCorDATA,filename_outputdir.Data(),useMomentumCorr,smearRochCorrByNsigma,useEffSF,usePileupSF,sample.Data()));
  // // gROOT->ProcessLine(Form("PhiStarEtaAnalysis zDATA(\"%s\",%f,%d)",WfileDATA.Data(),WfileDATA_lumi_SF,sample.Contains("WJetsSig")?useAlsoGenPforSig:0));
  ...
  // // gROOT->ProcessLine(Form(".! mv ../%s/PhiStarEtaAnalysis.root ../%s/WanalysisOnDATA.root",filename_outputdir.Data(),filename_outputdir.Data()));
  // // gROOT->ProcessLine(Form(".! cp PhiStarEtaAnalysis.\* ../JobOutputs/%s/",filename_outputdir.Data()));
